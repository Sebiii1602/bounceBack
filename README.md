...
- Momentum-Score: gleitender Wert 0–100, der sich dort einpendelt, wo deine Quote der letzten Wochen liegt (~83 bei 80 % on track). Die Stärke eines Ausrutschers (Leicht/Mittel/Deutlich — z. B. „wie weit über dem Kalorienbudget?“) verschiebt ihn spürbar, ohne ihn abstürzen zu lassen. Mit Kurve und In-App-Erklärung (ⓘ) im Trend-Tab
- **Perfekte Wochen**: sieben On-track-Tage am Stück — egal an welchem Wochentag sie anfangen. Auf der Heute-Karte und im Verlauf: die letzten 7 Tage als Kacheln, die laufende Serie und ein Lebenszeit-Zähler. Ein Sammelstand, kein Streak — er kann nur steigen, ein schlechter Tag beendet nur die Serie
...
- **Momentum:** startet bei 50; jeder geloggte Tag zieht den Wert um ein Zehntel in Richtung dieses Tages — on track zu 100, ein Ausrutscher zu 35/15/0 je nach Stärke (unbewertet = mittel). Nicht geloggte Tage frieren den Wert ein.

  Dadurch **pendelt er sich von selbst dort ein, wo deine Quote liegt**: ~83 bei 80 % on track, ~92 bei 90 %, ~58 bei 50 %. Die Asymmetrie entsteht dabei von allein — weit unten bringt ein guter Tag viel (+8) und ein Ausrutscher kostet fast nichts, weit oben ist es umgekehrt (+1 bzw. −7). Erholung aus dem Tief geht schnell, oben halten wird zäh.

  *Frühere Fassung, bewusst verworfen:* feste Beträge (`+2` / `−8`). Die hatte einen Kipppunkt bei 80 % Quote — wer knapp darunter lag, rutschte unaufhaltsam auf 0, auch bei objektiv guten 80 %. Ein Score, der nur zwei stabile Zustände kennt (0 oder 100), misst nichts.

  Die Stärke wirkt bewusst **nur** aufs Momentum — die 30-Tage-% bleiben binär (Richtung, nicht Ausmaß). Konstanten in [`src/lib/config.ts`](src/lib/config.ts).
- **Perfekte Wochen:** **sieben aufeinanderfolgende On-track-Tage**, unabhängig vom Wochentag — eine Serie von Mittwoch bis Dienstag zählt genauso wie Montag bis Sonntag. Längere Serien werden in Siebener-Blöcke geteilt und abgerundet: 13 Tage am Stück sind eine perfekte Woche, 14 sind zwei. Bewusst streng — die Schärfe ist der Punkt. Der Zähler ist ein Sammelstand über die gesamte Nutzungsdauer und geht **nie** runter: ein schlechter Tag nimmt nichts weg, er beendet nur die laufende Serie. Genau darin liegt der Unterschied zum Streak. Unterbrochen wird eine Serie von einem Ausrutscher genauso wie von einer Lücke — bei einem Tag ohne Eintrag ist schlicht nicht bekannt, wie er lief; nachtragen über den Kalender schließt sie jederzeit wieder.
